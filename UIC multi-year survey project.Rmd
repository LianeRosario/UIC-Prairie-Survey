...
```{r total-richness-change-year-to-year}
jwp_variables %>%
  group_by(plot) %>%
  arrange(year) %>%
  mutate(richness_change = total_richness - lag(total_richness),
         mean_richness_change = round(mean(richness_change, na.rm = T), digits = 0))
```

```{r change-mean-wt-c-year-to-year}
jwp_variables %>%
  group_by(plot) %>%
  arrange(year) %>%
  mutate(change_in_c = mean_wt_c - lag(mean_wt_c),
         mean_change_in_c = round(mean(change_in_c, na.rm = T), digits = 2))
```
...
jwp_species_frequency %>%
  ...
  mutate(change_species_freq = fraction- lag(fraction),
         mean_change_species_freq = round(mean(change_species_freq, na.rm = T), digits = 3))
```
