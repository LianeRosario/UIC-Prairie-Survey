...
```{r}
jwp_species_frequency$fraction_rounded <- round(jwp_species_frequency$fraction, digits = 3)
```
...
jwp_variables %>%
  ...
  mutate(richness_change = total_richness - lag(total_richness),
         mean_richness_change = mean(richness_change))
```
...
jwp_species_frequency %>%
  group_by(genus, species) %>%
  arrange(year) %>%
  mutate(change_species_freq = fraction_rounded - lag(fraction_rounded))
```
