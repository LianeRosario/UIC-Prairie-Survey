...
survey_1979 <- survey_1979 %>%
  left_join(usace_data, by = c("genus", "species"))
```
...
```{r create-c-value-mean}
usace_data_mean <- usace_data %>%
  group_by(genus) %>%
...
```{r join-mean-with-survey}
survey_1979 <- survey_1979 %>%
  left_join(usace_data_mean, by = c("genus"))
```
...
```{r remove-extra-c-value-columns}
survey_1979 <- survey_1979 %>%
  select(-c_value_mean)
```
